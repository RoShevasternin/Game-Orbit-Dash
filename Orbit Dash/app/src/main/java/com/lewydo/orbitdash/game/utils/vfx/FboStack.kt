...
object FboStack {
    ...
    // ------------------------------------------------------------------------
    // Покоління GL-контексту
    // ------------------------------------------------------------------------
    //
    //  Після повернення з фону Android може перестворити контекст. libGDX
    //  відновлює керовані ресурси, але вміст FBO зникає, а їхні
    //  colorBufferTexture стають НОВИМИ об'єктами; GL-only текстури
    //  (Texture(w, h, format) — знімки екрана) не відновлюються взагалі.
    //
    //  Замість реєстру чи обходу дерева — лічильник. GDXGame.resume() його
    //  інкрементує; кожен власник порівнює своє збережене значення в draw() і
    //  при розбіжності перестворює своє. Працює і для груп, яких зараз немає
    //  на сцені: вони перевірять при наступному показі.
    //
    /** Зростає на кожному resume(). Порівнюй зі своїм збереженим значенням у draw(). */
    var contextGeneration = 0
        private set

    /** Викликати з GDXGame.resume() — libGDX на той момент уже відновив керовані ресурси. */
    fun onContextLost() { contextGeneration++ }
}